...
public class Maximum_Sum_Of_Distinct_Subarrays_With_Length_K {
    public long maximumSubarraySum(int[] nums, int k) {
        ...
        HashMap<Integer, Integer> map = new HashMap<>(); // Map to store the frequency of each element in the current subarray
        ...
        while (i < nums.length) {
            ...
            map.put(nums[i], map.getOrDefault(nums[i], 0) + 1); // Update the frequency of the current element in the map
            ...
            if (i >= k - 1) { // Check if the window size has reached `k`
                ...
                map.put(nums[start], map.getOrDefault(nums[start], 0) - 1); // Decrease the frequency of the element at the start
